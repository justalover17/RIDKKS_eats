...
<head>
    ...
    <title>Ridkk's Eats — Add Food Item</title>
...
<div class="card">
    <div class="card-top">
        ...
        <h1>Add <em>New</em> Item</h1>
        <p>Fill in the details to add to the menu</p>
    </div>
    <div class="card-body">
        <%--
            MAPPING:
            action=insert       → FoodServlet.doPost() → insertFood()
            name="name"         → request.getParameter("name")
            name="price"        → Double.parseDouble(request.getParameter("price"))
            name="categoryId"   → Integer.parseInt(request.getParameter("categoryId"))
            name="description"  → request.getParameter("description") → food.setDescription()
            ${categories}       → categoryDao.fetchAllCategories() in showAddForm()
                                   Category needs getCategoryId() and getCategoryName()
        --%>
                ...
                <div class="form-group">
                    ...
                    <select id="categoryId" name="categoryId" required>
                        ...
                        <c:forEach var="cat" items="${categories}">
                            <%-- Try getCategoryName() first, fallback to getName() --%>
